...
- **Terminals no longer come back corrupted after the window loses and regains focus.** After switching away from the VSCode window (another app, lock screen, a different monitor) and returning, a terminal could render with stacked, overlapping status lines and the cursor landing near the top of the pane instead of at the prompt — recoverable only by resizing the window, clearing, or reopening the tab. While the window is in the background the editor pauses the terminal's renderer, so on return its view could be out of step with the running program. The extension now forces a redraw of its terminals whenever the window regains focus (the same redraw a manual resize would trigger), so the pane comes back clean and the cursor lands where it should.
