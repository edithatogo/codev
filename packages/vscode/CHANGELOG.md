...
- **Builders tree groups by stage by default.** The Builders tree now groups builders by their lifecycle stage (`SPECIFY → PLAN → IMPLEMENT → REVIEW → PR → VERIFIED`) instead of by `area/*` label. Each row reads `[<area>] #<id> <title>`. A title-bar toggle (`$(tag)` ↔ `$(milestone)`) flips back to area grouping with a `[<phase>]` row prefix; persisted per workspace via the new `codev.buildersGroupBy` setting. Backlog grouping is unchanged.
...
- **Click-to-reconnect affordance on terminal give-up.** The red give-up line carries a clickable `Reconnect` token. Click it for a fresh retry chain — repeatable across cycles. Works for both architect and builder terminals.
