...
The under-the-hood story this release credits is Mohid Makhdoomi's second contribution to Codev: PR #1081 fixes a long-standing argv-ordering bug in the gemini consult lane that was silently delivering the literal string `--sandbox` to `agy` as the prompt instead of the real consultation text. That single line of arg shuffling restores the third leg of CMAP-3 reviews, which had been quietly degraded to CMAP-2 across the entire v3.2.0 cycle.
