...
Another Jacobean-line patch focused on the VS Code extension. The biggest story is the Builders tree's reframe — grouping by lifecycle stage (`SPECIFY → PLAN → IMPLEMENT → REVIEW → PR → VERIFIED`) instead of `area/*`, so triage answers "what's blocking me right now?" instead of "what domain am I in?". Alongside it: area-header roll-up status icons, a Codev CLI preflight on startup with a guided walkthrough, and a new editor-tab Search Backlog webview. The cycle's biggest bug-fix wins close three long-standing annoyances — the terminal no longer spams "Connection lost" indefinitely (now with a one-click recovery affordance), the sidebar no longer blanks empty during transient connection blips, and the dev-server menu / keybindings / palette stop offering options for missing config.
